...
public class CrabDrive implements PIDOutput {
    ...
    public double dSetP;
    public double oldSetP;
    public double maxDSetP;
...
//    UltraSonicSensor rightBallSensor;
    ...
    CrabDrive() throws CANTimeoutException {
        gyro = new GyroSensor(TwentyTwelve.ROBOT_ANGLE_GYRO_SENSOR);
        front = new SteeringUnit(TwentyTwelve.FRONT_STEERING_CAN_ID,
                TwentyTwelve.FRONT_LEFT_CAN_ID,
                TwentyTwelve.FRONT_RIGHT_CAN_ID);
        rear = new SteeringUnit(TwentyTwelve.REAR_STEERING_CAN_ID,
                TwentyTwelve.REAR_LEFT_CAN_ID,
                TwentyTwelve.REAR_RIGHT_CAN_ID);
...
//        rightBallSensor = new UltraSonicSensor(ReboundRumble.RIGHT_BALL_RANGE_SENSOR);
    ...
    public void Drive(double outputValue) throws CANTimeoutException, NullPointerException {
        front.set(CalcWheelSetPoint(outputValue));
        rear.set(CalcWheelSetPoint(outputValue * -1.0));
    }
    ...
    /**
     * CalcPositionSetPoint()
     *
     * This method calculates the setpoint for the wheels using an exponential
     * function that is dependent on the wheel speed.
     *
     * @param currentSetP is the current set point for the wheel, where you want
     * the wheel to go
     * @param wheelSpeed is the speed of the wheel
     * @return newSetpoint is the actual setpoint for the wheels
     *
     * intDSetP is the intended change in position. maxDSetP is the actual
     * change in position.
     */
    public double CalcPositionSetPoint(double currentSetP, double wheelSpeed) {
        double intDSetP = math.Abs(currentSetP - oldSetP);
        if (intDSetP > 1.0) {
            intDSetP = 1.0;
        }
        wheelSpeed = math.Abs(wheelSpeed);
        if (wheelSpeed != 0.0) {
            maxDSetP = 1.0 / (wheelSpeed * 20);
        }
        if (wheelSpeed == 0.0 || intDSetP < 0.05) {
            maxDSetP = intDSetP;
        }
        if (currentSetP < oldSetP) {
            maxDSetP = maxDSetP * -1.0;
        }
        double newSetpoint = oldSetP + maxDSetP;
        oldSetP = newSetpoint;
        if (newSetpoint > 1.0) {
            newSetpoint = 1.0;
        }
    ...
    public double CalcWheelSetPoint(double wheelSpeed) {
        double intDSetP = math.Abs(wheelSpeed - oldSpeed);
        if (intDSetP > 1.0) {
            intDSetP = 1.0;
        }
        wheelSpeed = math.Abs(wheelSpeed);
        if (wheelSpeed != 0.0) {
            maxDSetP = 1.0 / (wheelSpeed * 10);
        }
        if (wheelSpeed == 0.0 || intDSetP < 0.05) {
            maxDSetP = intDSetP;
        }
        if (wheelSpeed < oldSpeed) {
            maxDSetP = maxDSetP * -1.0;
        }
        double newSetpoint = oldSpeed + maxDSetP;
        oldSpeed = newSetpoint;
        if (newSetpoint > 1.0) {
            newSetpoint = 1.0;
        }
        if (newSetpoint < -1.0) {
...
//    }
     ...
     *
     * @param joystickValue is the position to steer the wheels to in the range
     * of -1.0 ... 0.0 ... 1.0 where 0.0 is centered, 1.0 is ______ and -1.0 is
     * ______
     */
    public void CrabSteer(double joystickValue, double wheelSpeed) throws CANTimeoutException {
        front.Steer(CalcPositionSetPoint(joystickValue, wheelSpeed));
        rear.Steer(CalcPositionSetPoint(joystickValue, wheelSpeed));
    }
     ...
     * ______
     */
    public void SlewSteer(double outputValue, double wheelSpeed) throws CANTimeoutException {
        front.Steer(CalcPositionSetPoint(outputValue, wheelSpeed * 0.75));
        rear.Steer(CalcPositionSetPoint(outputValue, wheelSpeed * -0.75));
    }
    ...
    public void AdjustTurnSpeed(double joystickValue, double speed) {
        front.AdjustTurnSpeed(joystickValue, speed);
        rear.AdjustTurnSpeed(joystickValue, speed);
    }
}